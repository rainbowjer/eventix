...
<div class="container py-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="fw-bold"><i class="fas fa-heart text-danger"></i> My Favorite Events</h2>
                ...
                <div class="col-md-4 mb-4">
                    <div class="concert-card shadow-sm">
                        ...
                        <div class="favorite-btn">
                            <button class="btn btn-sm favorite-toggle" data-event-id="{{ $event->id }}" data-is-favorited="true">
    ...
    @else
        <div class="text-center py-5">
            ...
            <h4 class="mt-3 text-muted">No favorite events yet</h4>
            <p class="text-muted">Start exploring events and add them to your favorites!</p>
...
$(document).ready(function() {
    ...
    $('.favorite-toggle').click(function() {
        ...
        const card = btn.closest('.concert-card');

        $.post(`/events/${eventId}/favorites/toggle`)
            .done(function(response) {
                if (response.success) {
                    if (response.isFavorited) {
                        // Keep the heart red (already favorited)
                    } else {
                        // Remove from favorites - animate and remove card
                        card.fadeOut(300, function() {
                            card.remove();
                            // Check if no more cards
                            if ($('.concert-card').length === 0) {
